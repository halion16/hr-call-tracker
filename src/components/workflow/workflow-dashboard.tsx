                ...
                <div className="h-[500px] overflow-y-auto px-6 pb-4">
                  <div className="space-y-3">
                    {notifications.map((notification) => (
                      <div key={notification.id} className="flex items-start gap-3 p-3 border rounded-lg shadow-sm">
                        <div className="text-xl">{getPriorityIcon(notification.priority)}</div>
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-600 mb-2 leading-tight">{notification.message}</p>
                          <div className="flex items-center gap-4 text-xs text-gray-500">
                            <span>{new Date(notification.createdAt).toLocaleDateString('it-IT')}</span>
                            <span>{notification.type}</span>
                            {notification.autoGenerated && (
                              <Badge variant="outline" className="text-xs py-0 px-1">AUTO</Badge>
                            )}
                          </div>
                        </div>
                      </div>
                ...
                <div className="h-[500px] overflow-y-auto px-6 pb-4">
                  <div className="space-y-3">
                    {employees
                      ...
                      .map((employee) => (
                        <Card key={employee.id} className="border-l-4 border-l-red-500 shadow-sm">
                          <CardContent className="p-3">
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                <h3 className="font-semibold text-sm">{employee.nome} {employee.cognome}</h3>
                                <p className="text-xs text-gray-600 mb-2">{employee.dipartimento} - {employee.posizione}</p>
                              ...
                              <Badge variant="destructive" className="text-xs ml-2">ALTO RISCHIO</Badge>
                            </div>
                          </CardContent>
                        </Card>
                      ))}
