...
export class SmartNotificationService {
  ...
  async createAutoScheduledNotification(
    ...
    const notification: SmartNotification = {
      ...
      message: `È stata programmata automaticamente una call per ${new Date(call.dataSchedulata).toLocaleDateString('it-IT')} basata su: ${suggestion.reasoning.slice(1, 3).join(', ')}`,
      contextData: {
        callId: call.id,
  ...
  async createPerformanceAlertNotification(
    ...
    const notification: SmartNotification = {
      ...
      priority: priority,
      title: `⚠️ Alert Performance: ${employee.nome} ${employee.cognome}`,
  ...
  async createContractExpiryNotification(
    ...
    const notification: SmartNotification = {
      ...
      priority: priority,
      title: `📋 Contratto in Scadenza: ${employee.nome} ${employee.cognome}`,
      message: `Il contratto di ${employee.nome} ${employee.cognome} scadrà tra ${daysUntilExpiry} giorni (${new Date(employee.contractExpiryDate!).toLocaleDateString('it-IT')}). ${this.getContractActionMessage(daysUntilExpiry)}`,
      contextData: {
        employeeId: employee.id,
        daysUntilExpiry,
  ...
  async createOverdueCallNotification(
    ...
    const notification: SmartNotification = {
      ...
      type: 'overdue_call',
      priority: priority,
      title: `⏰ Call in Ritardo: ${employee.nome} ${employee.cognome}`,
      message: `La call programmata per ${new Date(call.dataSchedulata).toLocaleDateString('it-IT')} è in ritardo di ${daysPastDue} giorni. ${this.getOverdueActionMessage(daysPastDue)}`,
      contextData: {
        callId: call.id,
        employeeId: employee.id,
        daysPastDue,
  ...
  async createIntelligentCallReminder(
    ...
    const notification: SmartNotification = {
      ...
      type: 'call_reminder',
      priority: 'medium',
      title: `📞 Promemoria Call: ${employee.nome} ${employee.cognome}`,
  ...
  /**
   * Ottieni canali predefiniti basati sulla priorità
   */
  ...
  /**
   * Ottieni notifiche per priorità
   */
